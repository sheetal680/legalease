...
import { Scale, Home, UserCheck, Users, FileText, Menu, X, LogOut, PlusCircle } from 'lucide-react'
import { useAuth } from '../../context/AuthContext'

const NAV = [
  { label: 'Home',           to: '/advocate-home',            icon: Home,       end: true },
  { label: 'Add Associate',  to: '/advocate/add-associate',   icon: UserCheck },
  { label: 'Add Client',     to: '/advocate/add-client',      icon: PlusCircle },
  { label: 'My Clients',     to: '/advocate/clients',         icon: Users },
  { label: 'My Documents',   to: '/advocate/documents',       icon: FileText },
]
...
export default function AdvocateLayout({ children }) {
  ...
  const advocateName = profile?.full_name || user?.email?.split('@')[0] || 'Advocate'
    ...
    <div className="flex h-screen overflow-hidden bg-white">
      ...
      >
        ...
        <div className="flex items-center gap-2.5 px-5 py-5 shrink-0">
          ...
          <span className="text-[#c9a84c] font-bold text-base whitespace-nowrap leading-tight">
            LegalEase<br />
            <span className="text-[10px] font-semibold tracking-widest text-[#c9a84c]/70 uppercase">Advocate</span>
          </span>
        </div>
        ...
        <div className="px-4 py-4 shrink-0 border-t border-white/10">
          ...
          <p className="text-white text-sm font-semibold truncate mb-3">{advocateName}</p>
          <button
      ...
      <div className="flex-1 flex flex-col overflow-hidden min-w-0">
        ...
        <header className="shrink-0 flex items-center gap-3 px-6 py-4 border-b border-gray-100 bg-white">
          ...
          <span className="text-sm font-semibold text-gray-400">LegalEase Advocate Portal</span>
        </header>
